...
### Corrigido
- Reposts de vídeos do Instagram para Threads falhavam silenciosamente com `error_message=UNKNOWN` retornado pela Graph API. Causa raiz: o Instagram CDN re-encoda o áudio dos vídeos para HE-AAC para economizar banda; Meta Threads (e Reels) aceitam **somente AAC-LC** apesar da documentação dizer apenas "AAC". O mesmo vídeo postado manualmente (com áudio AAC-LC do encoder original do dispositivo) publicava normalmente. Foi adicionado novo helper `ensureAacLcAudio` em `libraries/.../upload/video.aaclc.ts` que faz probe do áudio com ffprobe, e se não for AAC-LC transcoda via ffmpeg (`-c:v copy -c:a aac -profile:a aac_low -b:a 128k -movflags +faststart`) — vídeo é só copiado, então a operação custa 1-3s para Story/Reel curto. A repost activity passou a baixar o vídeo IG, transcodar quando necessário, e fazer upload do resultado limpo via `storage.uploadFile`. Imagens e vídeos que já estão em AAC-LC não passam pelo transcode (fail-open, sem custo). `ffmpeg` foi adicionado ao `Dockerfile.dev` (apt-get install). Beneficia também os outros destinos que sejam mais rigorosos com encoding (Pinterest, X, etc.).
