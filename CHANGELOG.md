...
### Corrigido
- Badge do Zernio (overlay vermelho com o logo no canto superior direito do ícone do canal) escapava do ícone e aparecia no topo do dropdown ou do card pai, dependendo do contexto. Causa: `PlatformIconBadge` retornava um fragmento sem ancestor `position:relative`, então o `<span>` absoluto subia até o primeiro container posicionado (dropdown de seleção de destinos do Repost, lista de canais do calendário etc.). Agora o componente envolve o ícone num `<span class="relative inline-flex shrink-0">` com tamanho fixo, garantindo que o badge ancore sempre na borda do ícone — em qualquer dropdown, card ou linha. Visualmente fica claro de novo quais redes são publicadas via Zernio (TikTok, Pinterest etc.).
