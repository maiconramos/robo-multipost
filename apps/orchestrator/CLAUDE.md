...
| `src/workflows/refresh.token.workflow.ts` | Refresh OAuth **por-canal** — só é iniciado na conexão e só para providers com `refreshCron=true` (Threads, Instagram-standalone). **Ainda sem reconciliação no boot** (some num redeploy — ver pitfall #9) |
| `src/workflows/refresh.tokens.cron.workflow.ts` | Refresh OAuth proativo **em lote** — workflow SINGLETON diário (`refresh-tokens-cron`) que renova todo canal cujo token expira em ≤1 dia, cobrindo TODOS os providers (loop resiliente + `continueAsNew`; reconciliado no boot por `RefreshIntegrationService.ensureRefreshTokensCronWorkflow` — ver pitfall #9). Para providers `noNativeRefresh` (`facebook`/`instagram`), "renovar" agora significa self-heal via `MetaSystemUserService` (System User token configurado) ou skip silencioso (sem desconectar) — ver pitfall correspondente em [`libraries/nestjs-libraries/CLAUDE.md`](../../libraries/nestjs-libraries/CLAUDE.md) |
| `src/workflows/repost.workflow.ts` | Scheduled repost — loop resiliente: `try/catch` com backoff + `continueAsNew` a cada N ciclos; reconciliado no boot por `RepostService.reconcileWorkflows` (ver pitfall #9) |
...
9. **Symptom:** a long-running workflow (e.g. repost) **disappears** from Temporal after a deploy, or a user has to **re-save a rule** to recreate it. → **Cause:** an infinite `while (true)` loop workflow was fragile in three ways: (a) **no `continueAsNew`** → history grew unbounded until Temporal **force-terminated** it (~50k events / 50MB); (b) an **unhandled activity failure** (e.g. `getRuleFresh` throwing because Postgres wasn't ready right after a deploy) propagated and terminated the workflow; and (c) **nothing recreated it** on boot. → **Fix (canonical: repost):** a long-running loop workflow MUST (1) call `continueAsNew({ ...state })` every N cycles to bound history — the call goes **outside** any `try/catch` (it unwinds via a control-throw that a `catch` would swallow); see `send.email.workflow.ts` / `digest.email.workflow.ts` / `repost.workflow.ts`; (2) wrap the cycle body in `try/catch` with a backoff so a transient failure retries instead of killing the workflow (a genuine "rule disabled/deleted" result still exits); and (3) be re-ensured on boot via an idempotent reconciliation registered in `StartupMigrationService` — `RepostService.reconcileWorkflows()` calls `workflow.start(...)` with `workflowIdConflictPolicy: 'USE_EXISTING'` (no-op when already Running) for every enabled rule. `refresh.tokens.cron.workflow.ts` is a **second canonical example** of the full three-part fix (loop + `continueAsNew` + boot reconciliation via `RefreshIntegrationService.ensureRefreshTokensCronWorkflow()`, registered as `reconcileRefreshTokensCron` in `StartupMigrationService`). **NOTE:** `autopost.workflow.ts`, `missing.post.workflow.ts` and `refresh.token.workflow.ts` still carry the `while(true)`-without-`continueAsNew` anti-pattern — apply the same three-part fix when you touch them. In particular the **per-channel** `refresh.token.workflow.ts` still has **no boot reconciliation** (only started on connect), so it silently vanishes on a redeploy — a known follow-up; the new batch cron covers those providers proactively in the meantime.

10. **Symptom:** worker fails to bundle / non-deterministic workflow after touching an email/notification path → **Cause:** a `*.workflow.ts` imported the email i18n module (`@gitroom/nestjs-libraries/emails/i18n/*` — `emailT`/`CATALOG`/`renderDigestEmail`). Those render text and must stay **out of the workflow bundle** (same class as the `crypto` incident). **Fix:** workflows carry only translation **key + params** (see the `Email` type in `signals/email.signal.ts` and the `inAppNotification` payload); the actual rendering happens in the **activity** `EmailActivity.sendDigestEmail` (→ `renderDigestEmail`). `streak`/`digest` workflows pass `{subjectKey, messageKey, params, profileId}` through the signal and filter recipients by profile inline; they never call `emailT`. Full map: [`docs/architecture/email-i18n.md`](../../docs/architecture/email-i18n.md).

11. **Symptom:** a workflow has no poller after splitting orchestrator containers, or a provider worker wastes memory compiling workflow bundles → **Cause:** every workflow in this fork is started on the `main` task queue; provider queues exist only to execute activities. **Fix:** keep `workflowsPath` exclusively on the `main` worker. New workflow starts must continue using `taskQueue: 'main'` unless the architecture and worker topology are deliberately migrated together. `WORKER_CONCURRENCY_DIVIDER` divides each provider activity limit across equivalent containers; `EXCLUDE_QUEUE` assigns exact provider queues to selected containers and intentionally refuses `main`. Because `/health/workers` queries **workflow** pollers, keep `TEMPORAL_HEALTH_TASK_QUEUES` on `main`; provider activity-only queues are not valid entries for that probe.
