...
| `src/components/status/` | Admin-only "Status" area (`/status`, menu item gated `role: ['ADMIN','SUPERADMIN']` in `layout/top.menu.tsx`) — `status.component.tsx` is the tab shell with two tabs: `problems.component.tsx` (derived, per-profile problems — disconnected channels, failed posts, failed IG automations — disappears once resolved) and `history.component.tsx` (append-only `StatusEvent` log that survives resolution, cursor-paginated via `useSWRInfinite`, filterable by type). `status.helpers.tsx` holds the shared `ChannelAvatar`/`ProfileChip`/`DebugLink`/`temporalPostUrl` used by both tabs. Contract in `nestjs-libraries/src/dtos/status/status.dto.ts` (+ `status-history.query.dto.ts` for the paginated query) |
