...
14. **Symptom:** Kie.ai video keeps polling after moderation already rejected the task, or a request can hang on a stalled socket → **Cause:** treating any unknown/missing `successFlag` as “still generating” and calling `fetch` without a signal. **Fix:** only `successFlag=0` may continue; `1` must include a URL and every other state fails immediately. Keep `AbortSignal.timeout(30_000)` on create/poll, log the returned `taskId`, and sanitize both Bearer values and the exact resolved API key before exposing provider text. The enclosing `MediaService.generateAiVideo → useCredit` rejection path performs the existing compensating credit delete.
