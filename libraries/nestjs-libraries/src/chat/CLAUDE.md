...
8. **Symptom:** agent streaming error (e.g. provider out of credits) gives zero feedback — user sees nothing and the error only appears in the browser console → **Cause:** `<CopilotChat>` had no `ErrorMessage` prop and no `onError` handler. **Fix:** `agent.chat.tsx` now passes `ErrorMessage={AgentErrorMessage}` (inline red bubble with a classified pt-BR message) and `onError` (toaster fallback). The helpers `classifyAiError`/`friendlyAiErrorMessage`/`sanitize` in that component mirror the backend `buildFriendlyProviderMessage`/`sanitize` in `ai-text.service.ts`. If you add a second chat surface backed by `CopilotChat`, replicate both props — omitting either leaves the user with a silent failure.
...
9. **Symptom:** external MCP memory grows across connector conversations, or clients stop recovering after a backend restart → **Cause:** Streamable HTTP was switched back from Mastra stateless mode to `sessionIdGenerator`, retaining one transport/server per client session. **Fix:** keep `serverless: true` on `/mcp-oauth`, `/mcp`, and `/mcp/:id`; authentication remains request-scoped through `runWithContext`. Do not change the legacy SSE routes as part of this fix. OAuth discovery must remain limited to the path-inserted `/mcp-oauth` well-known URLs so `/mcp/:id` API-key connectors do not get forced into OAuth.
10. **Symptom:** an approved `generateVideoTool` call leaves the chat turn streaming forever when Kie rejects asynchronously → **Cause:** the tool lets `MediaService.generateAiVideo` throw through the Mastra run. **Fix:** return a controlled `{ error }` output for a known `HttpException` (generic text for unexpected exceptions), and keep `id`/`path` optional in the output schema. Never take `organizationId` or `profileId` from model input; preserve request context when wrapping the call.
