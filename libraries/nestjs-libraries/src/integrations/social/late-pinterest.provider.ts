...
export class LatePinterestProvider
  ...
  implements SocialProvider
{
  ...
  async generateAuthUrl(
    ...
    const redirectUrl = `${process.env.FRONTEND_URL}/integrations/social/late-pinterest`;
  ...
  async authenticate(params: {
    ...
    return {
      id: lateAccountId,
      name: account?.name || 'Pinterest Account',
      accessToken: lateApiKey,
      refreshToken: '',
      expiresIn: 999999999,
      picture: account?.picture || '',
      username: account?.username || account?.name || 'pinterest_user',
    };
  }
