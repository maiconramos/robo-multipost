...
| Payload/Workers Temporal | `cc79e129`, `5b0288fd`, `6e1c103e` | Porte dividido para isolar riscos. O limite de `ApplicationFailure` foi implementado primeiro, preservando códigos Meta e sem tocar filas/workflows; ver [`temporal-failure-payload-upstream-audit-2026-09.md`](./temporal-failure-payload-upstream-audit-2026-09.md). Workers activity-only e distribuição de concorrência seguem em PR própria, com validação das filas `main` e por-provider. |
